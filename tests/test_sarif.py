"""Tests for the SARIF 2.1.0 emitter in sdevpro.engine.report.sarif."""
...
def test_write_sarif_basic_shape(tmp_path: Path) -> None:
    ...
    run = doc["runs"][0]
    assert run["tool"]["driver"]["name"] == "SDeVPro"
...
def test_write_sarif_never_embeds_poc_script(tmp_path: Path) -> None:
    ...
    poc = _read(tmp_path)["runs"][0]["results"][0]["properties"]["sdevpro.engine"]["poc"]
...
def test_write_sarif_emits_version_control_provenance(tmp_path: Path) -> None:
    ...
    assert run["automationDetails"] == {"id": "sdevpro/engine/acme/widget"}
