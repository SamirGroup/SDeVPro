...
from sdevpro.engine.report.state import (
    ReportState,
...
def test_streaming_logging_stays_enabled_for_cost_callback() -> None:
    with (
        ...
        patch("sdevpro.engine.config.models._register_litellm_cost_callback") as register,
    ):
        _configure_litellm_compatibility()
...
def test_cost_callback_reads_openrouter_stream_usage_cost() -> None:
    ...
    with patch("sdevpro.engine.report.state.get_global_report_state", return_value=report_state):
...
def test_cost_callback_estimates_cost_with_provider_prefixed_model() -> None:
    ...
    with (
        patch("sdevpro.engine.report.state.get_global_report_state", return_value=report_state),
