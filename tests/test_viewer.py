"""Tests for the local run viewer (sdevpro.engine.interface.viewer) and its path helpers."""
...
from sdevpro.engine.core.paths import latest_run_dir, runs_base_dir
from sdevpro.engine.interface.viewer.server import serve
from sdevpro.engine.interface.viewer.transcript import (
    build_run_state,
...
def _make_run(base: Path, name: str, *, status: str, end_time: str | None) -> Path:
    run_dir = base / "sdevpro_runs" / name
    ...
    agents = {
        ...
        "names": {"root": "sdevpro.engine", "child": "recon"},
...
def test_latest_run_dir_none_when_no_runs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert runs_base_dir() == tmp_path / "sdevpro_runs"
...
def test_server_serves_api_and_static(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("sdevpro.engine.interface.viewer.server.bundle_dir", lambda: assets)
...
def test_server_event_endpoint_forwards_cta(
    ...
    monkeypatch.setattr(
        "sdevpro.engine.telemetry.posthog.viewer_cta_clicked",
...
def test_server_event_endpoint_forwards_email_funnel(
    ...
    seen: list[tuple[str, str | None]] = []
    monkeypatch.setattr(
        "sdevpro.engine.telemetry.posthog.viewer_email_event",
...
def test_server_event_endpoint_forwards_agent_steered(
    ...
    seen: list[bool] = []
    monkeypatch.setattr("sdevpro.engine.telemetry.posthog.viewer_agent_steered", lambda: seen.append(True))
...
def test_feedback_records_telemetry_on_success(
    ...
    sent: list[bool] = []
    monkeypatch.setattr("sdevpro.engine.interface.viewer.auth.feedback_submit", lambda *_a: None)
    monkeypatch.setattr(
        "sdevpro.engine.telemetry.posthog.viewer_feedback_submitted", lambda: sent.append(True)
    )
...
def test_auth_status_reflects_expiry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _bundle(tmp_path, monkeypatch)
    monkeypatch.setattr(
        "sdevpro.engine.interface.viewer.auth.read_auth", lambda: {"email": "a@b.com", "token": "t"}
    )
    verified = {"value": True}
    monkeypatch.setattr("sdevpro.engine.interface.viewer.auth.is_verified", lambda: verified["value"])
...
def test_auth_mutations_require_session(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("sdevpro.engine.interface.viewer.auth.forget", lambda: forgotten.update(value=True))
...
def test_runs_list_requires_session_and_verification(
    ...
    monkeypatch.setattr("sdevpro.engine.interface.viewer.auth.is_verified", lambda: True)
