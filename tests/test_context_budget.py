...
from sdevpro.engine.llm import context_budget
...
def test_context_window_chatgpt_prefix_skips_provider_auth(
    ...
    monkeypatch.setattr("sdevpro.engine.llm.context_budget.litellm.get_model_info", _model_info)
    try:
...
def test_context_window_unmapped_uses_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("sdevpro.engine.llm.context_budget.litellm.get_model_info", _raise)
...
def test_count_tokens_fallback_on_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("sdevpro.engine.llm.context_budget.litellm.token_counter", _raise)
