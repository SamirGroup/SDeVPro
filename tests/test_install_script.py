...
def _create_release_archive(tmp_path: Path) -> Path:
    binary_name = f"sdevpro.engine-{RELEASE_VERSION}-{RELEASE_TARGET}"
    binary_path = tmp_path / binary_name
    _write_executable(binary_path, f"#!/bin/sh\nprintf 'sdevpro.engine {RELEASE_VERSION}\\n'\n")
...
def test_installer_downloads_and_runs_linux_arm64_release(tmp_path: Path) -> None:
    ...
    expected_filename = f"sdevpro.engine-{RELEASE_VERSION}-{RELEASE_TARGET}.tar.gz"
    assert expected_filename in curl_log_path.read_text(encoding="utf-8")

    installed_binary = home_path / ".sdevpro/engine/bin/sdevpro.engine"
    ...
    assert installed_result.stdout.strip() == f"sdevpro.engine {RELEASE_VERSION}"
...
def test_installer_rejects_unsupported_architecture(tmp_path: Path) -> None:
    ...
    assert not (home_path / ".sdevpro.engine").exists()
