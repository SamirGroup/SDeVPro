...
from sdevpro.engine.config.tool_call_ids import TurnCallIdRewriter, dedupe_input
from sdevpro.engine.config.tool_call_limits import TurnToolCallLimiter
...
if TYPE_CHECKING:
    ...
    from sdevpro.engine.config.settings import LlmSettings, ReasoningEffort, Settings
...
class _NonStreamingModel(Model):
    ...
    SDeVPro uses only issues streamed requests, so such a gateway fails every
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply SDeVPro config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # SDeVPro uses LiteLLM's success callback to capture provider-reported cost.
...
def _install_openrouter_stream_cost_capture() -> None:
    ...
    lost and SDeVPro falls back to a cost-map estimate that is missing entirely
    ...
    from sdevpro.engine.report.state import streamed_openrouter_costs
...
OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://sdevpro.engine.ai",
    "X-Title": "SDeVPro",
...
def _register_litellm_cost_callback() -> None:
    ...
    from sdevpro.engine.report.state import litellm_cost_callback
