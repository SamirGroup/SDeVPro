...
from sdevpro.engine.utils.secret_files import write_secret_text
...
AUTH_PATH = Path.home() / ".sdevpro" / "subscription-auth.json"
...
def _refresh_guard() -> Iterator[None]:
    """Serialize token refresh within (lock) and across (flock) SDeVPro processes,
...
def get_valid_token() -> tuple[str, str]:
    ...
    record = read_record()
    if record is None:
        raise CodexAuthError("not_authenticated", "not signed in; run: sdevpro.engine auth login")
...
def build_openai_client() -> AsyncOpenAI:
    ...
    return AsyncOpenAI(
        api_key="sdevpro-codex-oauth",  # placeholder; the hook overwrites Authorization
