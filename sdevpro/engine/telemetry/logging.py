...
_TRACKED_ROOTS: tuple[str, ...] = ("sdevpro.engine", "openai.agents")
...
def configure_dependency_logging() -> None:
    """Quiet dependency logging/warnings that obscure SDeVPro scan logs."""
...
def setup_scan_logging(run_dir: Path, *, debug: bool | None = None) -> Callable[[], None]:
    ...
    Args:
        run_dir: Per-scan output directory. ``{run_dir}/sdevpro.engine.log`` is
    ...
    log_path = run_dir / "sdevpro.engine.log"
