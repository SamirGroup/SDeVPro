"""SARIF 2.1.0 output for SDeVPro vulnerability reports.

Builds a GitHub code-scanning compatible SARIF document from SDeVPro findings
...
Design notes:
  * Rules are keyed on CWE (``id = CWE-NNN``), falling back to CVE, then
    ...
    SDeVPro output variants (``CWE-306``, ``cwe: 306``, ``306``) to the
    canonical ``CWE-NNN`` form so dedup works across runs.
  * SARIF only has three levels (error / warning / note). SDeVPro's five
    severities collapse into them. The raw severity label and CVSS score
    survive in ``result.properties.sdevpro.engine`` for downstream tools that can
...
SARIF_VERSION = "2.1.0"
TOOL_NAME = "SDeVPro"
TOOL_INFORMATION_URI = "https://sdevpro.engine.ai"
...
# SARIF only has three result levels; SDeVPro's five severities collapse here.
# Original label survives in ``result.properties.sdevpro.engine.severity``.
_SEVERITY_TO_LEVEL = {
    "critical": "error",
    "high": "error",
    "medium": "warning",
...
# sdevpro-triage's DEFAULT_STRIDE_LEGS convention).
...
def _apply_repository_context(run: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    ``automationDetails.id`` categorises the run (``sdevpro/engine/<owner>/<repo>``)
    so multiple SDeVPro runs against the same repo reconcile rather than pile
    ...
    if full_name:
        run["automationDetails"] = {"id": f"sdevpro/engine/{full_name}"}
...
def _build_rule(rule_id: str, report: dict[str, Any]) -> dict[str, Any]:
    """Build a SARIF rule descriptor from a SDeVPro finding."""
...
def _result_properties(
    ...
    *,
    ...
    """SDeVPro-specific metadata for downstream consumers.

    The top-level ``security-severity`` matches GitHub code-scanning's
    expected property. SDeVPro-specific fields are namespaced under
    ``sdevpro.engine`` so generic SARIF consumers don't see them by default.
    """
    properties: dict[str, Any] = {
        "security-severity": _security_severity(report),
    }
    if class_fingerprint:
        ...
        # filter alerts by it without parsing the nested sdevpro.engine.* tree.
        properties["strix_vuln_class_hash"] = class_fingerprint
    if is_synthetic:
        # Top-level so reviewers + downstream automation can filter
        # synthetic-anchored alerts without parsing the nested sdevpro.engine.*
        # tree.
        properties["synthetic_location"] = True

    sdevpro.engine: dict[str, Any] = {}
    for key in (
        "id",
        "severity",
        "cvss",
    ...
    if poc_description or poc_script:
        ...
        sdevpro.engine["poc"] = poc

    if sdevpro.engine:
        properties["sdevpro.engine"] = sdevpro.engine
...
def _build_fixes(report: dict[str, Any]) -> list[dict[str, Any]] | None:
    ...
    SDeVPro findings carry the suggested change inline on each code
...
def _rule_id(report: dict[str, Any]) -> str:
    ...
    CWE values are normalised from SDeVPro output variants (``CWE-306``,
    ...
    title = _string_value(report.get("title")) or "sdevpro-finding"
    ...
    *,
    ...
    Computed from primitives that don't depend on LLM prose stability:
      ...
      - first SARIF location's URI + startLine — these come from
        SDeVPro's ``code_locations[].file`` and ``start_line`` which
    ...
    if uri:
        ...
        if start_line is not None:
            ...
            # line because SDeVPro code_locations carry the SINK line,
...
def _sarif_level(severity: Any) -> str:
    """Map SDeVPro severity labels to SARIF result levels."""
...
def _slugify(value: str) -> str:
    ...
    return slug or "sdevpro-finding"
