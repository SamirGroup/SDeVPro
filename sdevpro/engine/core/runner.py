...
from sdevpro.engine.agents.factory import build_strix_agent, make_child_factory
...
from sdevpro.engine.core.hooks import BudgetExceededError, ReportUsageHooks, recomputed_budget_flags
from sdevpro.engine.core.inputs import (
    build_root_task,
    build_scope_context,
    make_model_settings,
)
...
from sdevpro.engine.telemetry.logging import set_scan_id, setup_scan_logging
...
if TYPE_CHECKING:
    ...
    from sdevpro.engine.runtime.status import StatusSink
...
async def run_strix_scan(
    *,
    ...
    """Run or resume one SDeVPro scan against a sandbox.
    ...
    :func:`sdevpro.engine.runtime.session_manager.create_or_reuse`.
    ...
    logger.info(
        "%s SDeVPro scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    from sdevpro.engine.tools.notes.tools import hydrate_notes_from_disk
    from sdevpro.engine.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        logger.warning(
            ...
            "Resume with 'sdevpro.engine --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("SDeVPro scan %s failed", scan_id)
        if root_id is not None:
            with contextlib.suppress(Exception):
        ...
        logger.info("SDeVPro scan %s done", scan_id)
        teardown_logging()
