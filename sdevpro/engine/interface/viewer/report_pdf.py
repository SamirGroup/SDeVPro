...
The layout mirrors the SDeVPro cloud pentest report (cover page, executive
...
the user's own hands. SDeVPro cannot read the delivered report.
"""
...
class _LogoMark(Flowable):  # type: ignore[misc]  # reportlab base is untyped
    """The rounded-square SDeVPro mark drawn inline (no raster asset to ship)."""
...
def _cover(
    ...
    header = Table(
        [[_LogoMark(30), Paragraph("SDeVPro", styles["wordmark"])]],
...
def build_encrypted_report(run_dir: Path) -> tuple[bytes, str, str]:
    ...
    filename = f"sdevpro-report-{run_name}.pdf"
