"""`sdevpro.engine view [<run>]` command: serve a run's viewer UI locally."""
...
from sdevpro.engine.core.paths import (
    RUNS_DIR_NAME,
    latest_run_dir,
    run_dir_for,
    run_record_path,
...
from sdevpro.engine.interface.viewer.server import authorized_url, bundle_is_built, serve
from sdevpro.engine.interface.viewer.transcript import read_run_summary
...
def run_view(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="sdevpro.engine view",
        description="Open a local web view of a SDeVPro run (live or finished).",
    )
    parser.add_argument(
        "run",
        nargs="?",
    ...
    if not bundle_is_built():
        console.print(
            ...
            "Build it with: [cyan]cd sdevpro/engine/interface/viewer/frontend && npm ci && npm run build[/]"
        )
        raise SystemExit(1)
