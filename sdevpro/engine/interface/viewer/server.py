...
from sdevpro.engine.core.paths import run_record_path
...
class _ViewerState:
    def __init__(
        ...
        # The sdevpro_runs directory that holds the launched run; used to
        ...
        # standalone ``sdevpro.engine view`` / finished runs, so steering is unavailable.
    ...
    class ViewerHandler(BaseHTTPRequestHandler):
        ...
        def _handle_event(self) -> None:
            ...
            if event == "cta_clicked":
                from sdevpro.engine.telemetry import posthog
        ...
        def _handle_report_send(self) -> None:
            ...
            from sdevpro.engine.interface.viewer.report_pdf import build_encrypted_report
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``sdevpro.engine view``
    ...
    ``None`` (standalone ``sdevpro.engine view``), steering is reported unavailable.
    """
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="sdevpro-viewer", daemon=True)
    thread.start()
