...
from sdevpro.engine.config import codex, load_settings, persist_current
from sdevpro.engine.core.paths import run_dir_for
from sdevpro.engine.interface.cli_args import parse_arguments
from sdevpro.engine.interface.environment import (
    check_docker_installed,
    pull_docker_image,
    validate_environment,
)
from sdevpro.engine.interface.interactive import (
    InteractiveSetupUnavailableError,
    run_tui,
)
from sdevpro.engine.interface.scan_setup import (
    ModelConnectionError,
    preflight_model_connection,
    prepare_run,
    telemetry_start,
)
from sdevpro.engine.interface.update_check import (
    is_binary_install,
    notify_update,
...
from sdevpro.engine.telemetry.logging import configure_dependency_logging


BEDROCK_MODEL_PREFIX = "bedrock/"
BEDROCK_MISSING_MODULE_ERROR = "No module named 'boto3'"
BEDROCK_EXTRA_HINT = (
    'Bedrock support is optional. Install it with: pipx install "sdevpro-agent[bedrock]"'
)
VERTEX_MODEL_MARKER = "vertex"
VERTEX_MISSING_MODULE_ERROR = "No module named 'google"
VERTEX_EXTRA_HINT = (
    'Vertex AI support is optional. Install it with: pipx install "sdevpro-agent[vertex]"'
)
...
def _subscription_error_hint(exc: BaseException) -> str | None:
    ...
    if "not supported when using codex with a chatgpt account" in joined:
        return (
            ...
            "Set SDEVPRO_LLM to a model your plan includes (e.g. chatgpt/gpt-5.4)."
        )
    if (
        ...
        return (
            ...
            "  sdevpro.engine auth login chatgpt"
        )
    return None
...
async def warm_up_llm(show_model_warning: bool = True) -> None:
    ...
    from sdevpro.engine.core.inputs import make_model_settings
    ...
    try:
        ...
        if show_model_warning and raw_model and not is_recommended_or_frontier_model(raw_model):
            ...
            warn_text.append(f"'{raw_model}'", style="bold cyan")
            warn_text.append(
                " is not a recommended frontier model for SDeVPro.\nSecurity scans work best with:\n",
                style="white",
            )
        ...
        if settings.dedupe.model:
            from sdevpro.engine.report.dedupe import _dedupe_extra_args
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    from sdevpro.engine.report.state import get_global_report_state
    ...
    view_text.append("    ")
    view_text.append(f"sdevpro.engine view {args.run_name}", style="#22c55e")
    panel_parts.extend(["\n", view_text])
    ...
    if not scan_completed:
        ...
        resume_text.append("  ")
        resume_text.append(f"sdevpro.engine --resume {args.run_name}", style="#22c55e")
        panel_parts.extend(["\n", resume_text])
    ...
    console.print()
    console.print(
        "[#60a5fa]sdevpro.engine.ai[/]  [dim]·[/]  "
        "[#60a5fa]docs.sdevpro.engine.ai[/]  [dim]·[/]  "
        "[#60a5fa]discord.gg/sdevpro-ai[/]"
    )
    console.print()
...
def main() -> None:
    ...
    # `sdevpro.engine view [<run>]` is a viewer-only subcommand, dispatched before the
    # scan argument parser (which requires a target) and before any scan setup.
    if len(sys.argv) > 1 and sys.argv[1] == "view":
        from sdevpro.engine.interface.viewer.cli import run_view

        run_view(sys.argv[2:])
        return

    # `sdevpro.engine auth …` manages model-subscription sign-in and exits; it needs no
    # target, Docker, or scan setup.
    if len(sys.argv) > 1 and sys.argv[1] == "auth":
        from sdevpro.engine.interface.auth_cli import run_auth
    ...
    try:
        if args.non_interactive:
            from sdevpro.engine.interface.cli import run_cli
