"""Update notifications and self-update for the sdevpro.engine CLI.
...
result in ``~/.sdevpro.engine``, a non-intrusive notice with the upgrade command
for the detected install method, and a ``sdevpro.engine --update`` self-update
...
from sdevpro.engine.telemetry._common import get_version
...
_CACHE_PATH = Path.home() / ".sdevpro" / "update-check.json"
...
def get_upgrade_command(method: str | None = None) -> str:
    ...
    commands = {
        "binary": "sdevpro.engine --update",
        "pipx": "pipx upgrade sdevpro-agent",
        "uv": "uv tool upgrade sdevpro-agent",
        "pip": "pip install --upgrade sdevpro-agent",
    }
...
def notify_update(console: Console) -> None:
    ...
    if not latest:
        return
    console.print(
        f"[#eab308]A new version of sdevpro.engine is available:[/] "
...
def run_package_upgrade(console: Console, method: str) -> bool:
    ...
    if result.returncode != 0:
        ...
        return False
    console.print("[#22c55e]✓ sdevpro.engine updated — restart the scan to use the new version[/]")
    return True
...
def prompt_update_if_available(console: Console) -> bool:
    ...
    Returns True if sdevpro.engine was updated (caller should re-exec / exit).
    """
    latest = get_available_update()
    ...
    choice = Prompt.ask("Update sdevpro.engine?", choices=["y", "n", "s"], default="n")
    console.print()
    if choice == "s":
        skip_version(latest)
        return False
...
def _download_and_replace(version: str, target: str, console: Console) -> bool:
    ...
    filename = f"sdevpro.engine-{version}-{target}{archive_ext}"
    url = f"https://github.com/{GITHUB_REPO}/releases/download/v{version}/{filename}"
    binary_name = f"sdevpro.engine-{version}-{target}" + (".exe" if is_windows else "")
...
def self_update(console: Console | None = None, version: str | None = None) -> bool:
    ...
    if not is_binary_install():
        method = get_install_method()
        console.print(
            f"[#eab308]This sdevpro.engine was installed via {method};[/] "
    ...
    if not latest:
        console.print("[bold red]Could not determine the latest sdevpro.engine version.[/]")
        return False

    current = get_version()
    if current != "unknown" and not _is_newer(latest, current):
        console.print(f"[#22c55e]sdevpro.engine {current} is already the latest version.[/]")
        return True
    ...
    try:
        ...
        console.print(
            ...
            "[#60a5fa]curl -sSL https://sdevpro.engine.ai/install | bash[/]"
        )
        return False

    _write_cache(latest_version=latest, checked_at=time.time())
    console.print(f"[#22c55e]✓ Updated sdevpro.engine to {latest}[/]")
    return True
