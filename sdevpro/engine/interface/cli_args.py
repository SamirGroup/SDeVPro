"""Command-line argument parsing for the ``sdevpro.engine`` scan entrypoint."""
...
from sdevpro.engine.config import apply_config_override
from sdevpro.engine.config.settings import DEFAULT_MAX_TURNS
from sdevpro.engine.core.paths import run_dir_for, runtime_state_dir
from sdevpro.engine.interface.scan_setup import attach_workspace_mount, build_targets_info
from sdevpro.engine.interface.update_check import self_update
from sdevpro.engine.interface.utils import (
    check_mountable_dir,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SDeVPro Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  sdevpro.engine --target https://example.com

  # GitHub repository analysis
  sdevpro.engine --target https://github.com/user/repo
  sdevpro.engine --target git@github.com:user/repo.git

  # Local code analysis
  sdevpro.engine --target ./my-project

  # API spec test (OpenAPI/Swagger file or Postman collection export)
  sdevpro.engine --target ./openapi.yaml --target https://api.example.com
  sdevpro.engine --target ./collection.postman_collection.json

  # Postman collection pulled live by id (needs POSTMAN_API_KEY); optional environment
  sdevpro.engine --target postman://<collection-uuid> --target https://api.example.com
  sdevpro.engine --target "postman://<collection-uuid>?env=<environment-uuid>"

  # Domain penetration test
  sdevpro.engine --target example.com

  # IP address penetration test
  sdevpro.engine --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  sdevpro.engine --target https://github.com/user/repo --target https://example.com
  sdevpro.engine --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  sdevpro.engine --target-list ./targets.txt

  # Custom instructions (inline)
  sdevpro.engine --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  sdevpro.engine --target example.com --instruction-file ./instructions.txt
  sdevpro.engine --target https://app.com --instruction-file /path/to/detailed_instructions.md

  # Extra files placed in the sandbox workspace
  sdevpro.engine --target ./my-project --workspace-file ./wordlist.txt
  sdevpro.engine --target https://app.com --workspace-file ./openapi.yaml:specs/openapi.yaml
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"sdevpro.engine {get_version()}",
    )

    parser.add_argument(
        "--update",
        action="store_true",
        help="Update sdevpro.engine to the latest version and exit. Self-updates the "
...
def _load_resume_state(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    from sdevpro.engine.report.writer import read_run_record
