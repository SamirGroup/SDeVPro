"""UI-independent state and command controller for interactive SDeVPro clients."""
...
from sdevpro.engine.config.models import is_recommended_or_frontier_model
...
from sdevpro.engine.interface.tui.backend.projection import (
    MAX_TERMINAL_EVENTS,
...
from sdevpro.engine.interface.utils import is_subscription_run
...
class TuiController:
    ...
    async def _start(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        model = (load_settings().llm.model or "").strip()
        if not model:
            raise ValueError("No model configured. Set SDEVPRO_LLM first.")
    ...
    async def _open_viewer(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from sdevpro.engine.interface.tui.backend.messages import (
                send_user_message_to_agent,
            )
            from sdevpro.engine.interface.viewer.server import (
                authorized_url,
                bundle_is_built,
                serve,
            )
