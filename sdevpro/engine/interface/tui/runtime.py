...
from sdevpro.engine.config import load_settings, persist_current
from sdevpro.engine.core.agents import AgentCoordinator
from sdevpro.engine.core.hooks import BudgetExceededError
from sdevpro.engine.core.runner import run_strix_scan
...
from sdevpro.engine.interface.tui.backend import TuiBackendServer, TuiController
from sdevpro.engine.interface.tui.backend.live_view import TuiLiveView
from sdevpro.engine.interface.tui.sidecar import (
    check_return_code,
    child_environment,
    launch_tui_process,
    package_version,
...
from sdevpro.engine.interface.utils import read_workspace_files
from sdevpro.engine.report.state import ReportState, set_global_report_state
...
class GoTuiRuntime:
    ...
    async def _run_scan(self) -> None:
        image = str(load_settings().runtime.image or "sdevpro-sandbox:latest")
        try:
    ...
    def binary_command() -> list[str]:
        ...
        if (source / "go.mod").is_file() and shutil.which("go"):
            return ["go", "run", "./cmd/sdevpro-tui"]
        packaged = get_strix_resource_path("bin", tui_executable())
        if packaged.is_file():
            return [str(packaged)]
        raise RuntimeError(
            "Bubble Tea TUI binary not found. Reinstall SDeVPro from an official platform wheel."
        )
