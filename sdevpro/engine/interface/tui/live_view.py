...
from sdevpro.engine.interface.tui.history import load_session_history
...
_INTERNAL_TURN_PREFIXES = (
    # sdevpro.engine.core.agents._message_to_session_item: everything the coordinator
    # delivers from another agent or from the system, which wraps the stall,
    # terminal and budget-extension notices in sdevpro.engine.core.execution too.
    "[Message from ",
    # sdevpro.engine.core.inputs.child_initial_input: a subagent's parent context.
    "== Inherited context from parent",
    # sdevpro.engine.core.execution: the no-tool-call recovery nudge, both modes.
    ...
    # sdevpro.engine.core.hooks: budget warnings, the only notices injected unwrapped.
    *(
        f"[{label}] {subject}"
