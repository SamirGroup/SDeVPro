...
telemetry live here so ``sdevpro.engine.interface.main`` (the CLI) and
``sdevpro.engine.interface.tui.runtime`` (interactive setup) depend on one module
...
from sdevpro.engine.config import Settings, codex, load_settings
...
from sdevpro.engine.utils.api_spec import (
    SpecParseError,
...
async def preflight_model_connection(
    ...
    *,
    ...
    from sdevpro.engine.config.models import StrixProvider, configure_sdk_model_defaults
...
def _persist_run_record(args: argparse.Namespace) -> None:
    from sdevpro.engine.report.writer import write_run_record
