...
from sdevpro.engine.utils.api_spec import detect_spec_format
...
def _resolve_repo_diff_scope(
    ...
    if _is_repo_shallow(repo_path):
        raise ValueError(
            "SDeVPro requires full git history for diff-scope. Please set fetch-depth: 0 "
            "in your CI config."
        )
...
def check_docker_connection() -> Any:
    try:
        ...
        error_text.append(
            "Please ensure Docker Desktop is installed and running, and try running sdevpro.engine again.\n",
            style="white",
        )
