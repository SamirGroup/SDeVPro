"""`sdevpro.engine auth` — ChatGPT subscription sign-in (login / status / logout).

Signing in only stores OAuth tokens (``~/.sdevpro/subscription-auth.json``); model
selection stays with ``SDEVPRO_LLM``. A ``chatgpt/<model>`` SDEVPRO_LLM runs on the
subscription.
"""
...
from sdevpro.engine.config import codex, load_settings
...
_USAGE = "Usage:\n  sdevpro.engine auth login chatgpt [--manual]\n  sdevpro.engine auth status\n  sdevpro.engine auth logout"


def run_auth(argv: list[str]) -> int:
    """Entry point for ``sdevpro.engine auth …``. Returns a process exit code."""
    console = Console()
    # Bare `sdevpro.engine auth` (no subcommand) defaults to login.
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="sdevpro.engine auth login", add_help=True)
    parser.add_argument(
        "provider",
        nargs="?",
...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print("[yellow]Not signed in.[/] Run [cyan]sdevpro.engine auth login chatgpt[/] to sign in.")
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        console.print(f"  Runs use the subscription (SDEVPRO_LLM=[bold]{settings.llm.model}[/]).")
    else:
        console.print(
            "  [yellow]Note:[/] set [cyan]SDEVPRO_LLM[/] to e.g. [cyan]chatgpt/gpt-5.4[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("SDEVPRO_LLM", style="bold white")
    ...
    text.append("sdevpro.engine --target https://example.com", style="bold cyan")
    console.print()
    console.print(
        Panel(
            text,
...
def _logo_img_tag() -> str:
    """Return an ``<img>`` for the SDeVPro logo as an inline data URI, or "".
...
<body>
  <a class="topbar" href="https://sdevpro.engine.ai" target="_blank" rel="noopener"
     aria-label="SDeVPro — sdevpro.engine.ai">
    <!--LOGO-->
    <span>SDeVPro</span>
  </a>
  <div class="brand">SDeVPro</div>
  <h1>You're signed in</h1>
  <main class="card">
    <div class="badge">✓</div>
    <p class="msg">SDeVPro is connected to your ChatGPT subscription. Head back to your
      terminal — your security test runs there.</p>
    <div class="rule"></div>
    <p class="tagline">Autonomous AI hackers that <b>find and fix</b> your app's
      vulnerabilities.</p>
    <nav class="links">
      <a href="https://sdevpro.engine.ai" target="_blank" rel="noopener">sdevpro.engine.ai</a>
      <span class="dot">·</span>
      <a href="https://docs.sdevpro.engine.ai" target="_blank" rel="noopener">docs</a>
      <span class="dot">·</span>
      <a href="https://discord.gg/sdevpro-ai" target="_blank" rel="noopener">community</a>
    </nav>
  </main>
