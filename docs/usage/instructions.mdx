...
description: "Guide SDeVPro with custom testing instructions"
---
...
```bash
sdevpro --target https://app.com --instruction "Focus on authentication vulnerabilities"
```
...
```bash
sdevpro --target https://app.com --instruction-file ./pentest-instructions.md
```
...
```bash
sdevpro --target https://api.example.com \
...
<Tip>
Be specific. Good instructions help SDeVPro prioritize the most valuable attack paths.
</Tip>

## Workspace files

Instructions become part of the prompt. To give SDeVPro a file to work with, such
as a wordlist, an API specification, or notes, use `--workspace-file`. SDeVPro
places the file into the sandbox workspace before the scan starts.

```bash
sdevpro --target https://app.com --workspace-file ./wordlist.txt
```
...
Repeat the option for every file you want to place. SDeVPro lists the files in the
...
- The destination must not fall inside a target directory, because target files
  come from the target itself. SDeVPro skips such a file and logs a warning.
