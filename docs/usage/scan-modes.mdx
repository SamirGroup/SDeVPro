...
SDeVPro offers three scan modes to balance speed and thoroughness.

## Quick

```bash
sdevpro --target ./app --scan-mode quick
```
...
```bash
sdevpro --target ./app --scan-mode standard
```
...
```bash
sdevpro --target ./app --scan-mode deep
```
