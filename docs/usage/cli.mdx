---
title: "CLI Reference"
description: "Command-line options for SDeVPro"
---

## Basic Usage

```bash
sdevpro (--target <target> | --target-list <path>) [options]
```
...
<ParamField path="--target, -t" type="string">
  ...
  When the target is an API spec, SDeVPro copies it into the agent's workspace and authorizes the base URLs it declares (including those resolved from a Postman environment) as in-scope hosts - so the agent reads the contract and tests the full declared surface instead of discovering endpoints by crawling. Pair the spec with the deployed base URL (e.g. `--target ./openapi.yaml --target https://api.example.com`) so the agent has a reachable host to attack.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.sdevpro/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, SDeVPro enables streaming success callbacks to
...
```bash
# Basic scan
sdevpro --target https://example.com

# Authenticated testing
sdevpro --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
sdevpro --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
sdevpro -n --target ./ --scan-mode quick

# Cap cost and per-agent turns
sdevpro --target https://example.com --max-budget 25 --max-turns 300

# Force diff-scope against a specific base ref
sdevpro -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
sdevpro -t https://github.com/org/app -t https://staging.example.com

# API spec + live target (OpenAPI/Swagger file or Postman collection)
sdevpro -t ./openapi.yaml -t https://api.example.com

# Postman collection pulled live by id (+ optional environment)
sdevpro -t "postman://<collection-uuid>?env=<environment-uuid>"

# Targets from a file
sdevpro --target-list ./targets.txt

# Extra files placed in the sandbox workspace
sdevpro --target ./my-project --workspace-file ./wordlist.txt
sdevpro --target https://app.com --workspace-file ./openapi.yaml:specs/openapi.yaml
```
