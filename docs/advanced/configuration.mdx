---
title: "Configuration"
description: "Environment variables for SDeVPro"
---

Configure SDeVPro using environment variables or a config file.
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, SDeVPro keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, SDeVPro still writes complete run telemetry locally to:

```bash
sdevpro_runs/<run_name>/events.jsonl
```

When remote vars are set, SDeVPro dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="STRIX_IMAGE" default="ghcr.io/usestrix/sdevpro-sandbox:1.3.0" type="string">
...
SDeVPro stores configuration in `~/.sdevpro/cli-config.json`. You can also specify a custom config file:

```bash
sdevpro --target ./app --config /path/to/config.json
```
